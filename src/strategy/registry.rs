...
use super::Strategy;
use super::strategies::{autopilot, mean_reversion, momentum, tp_sl};
...
pub(crate) fn available() -> Vec<StrategyMeta> {
    vec![
        ...
        StrategyMeta {
            kind: "autopilot",
            summary: "Autopilot: condition-triggered entries (range/direction/window) with TP/SL.",
            default_params: serde_json::to_value(autopilot::Params::default())
                .unwrap_or(Value::Null),
        },
    ]
}
...
pub(crate) fn build(kind: &str, params: &Value) -> Result<Box<dyn Strategy>> {
    match kind {
        ...
        "autopilot" => {
            let p: autopilot::Params = parse(params)?;
            Ok(Box::new(autopilot::Autopilot::new(p)))
        }
